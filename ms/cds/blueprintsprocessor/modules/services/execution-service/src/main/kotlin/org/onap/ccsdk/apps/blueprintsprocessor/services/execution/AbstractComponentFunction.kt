...
abstract class AbstractComponentFunction : BlueprintFunctionNode<ExecutionServiceInput, ExecutionServiceOutput> {
    ...
    override fun prepareRequest(executionRequest: ExecutionServiceInput): ExecutionServiceInput {
        ...
        nodeTemplateName = this.operationInputs.getAsString(org.onap.ccsdk.cds.controllerblueprints.core.BluePrintConstants.PROPERTY_CURRENT_NODE_TEMPLATE)
        check(nodeTemplateName.isNotEmpty()) { "couldn't get NodeTemplate name for step($stepName)" }

        interfaceName = this.operationInputs.getAsString(org.onap.ccsdk.cds.controllerblueprints.core.BluePrintConstants.PROPERTY_CURRENT_INTERFACE)
        check(interfaceName.isNotEmpty()) { "couldn't get Interface name for step($stepName)" }

        operationName = this.operationInputs.getAsString(org.onap.ccsdk.cds.controllerblueprints.core.BluePrintConstants.PROPERTY_CURRENT_OPERATION)
    ...
    override fun prepareResponse(): ExecutionServiceOutput {
        ...
        status.code = 200
        status.message = org.onap.ccsdk.cds.controllerblueprints.core.BluePrintConstants.STATUS_SUCCESS
