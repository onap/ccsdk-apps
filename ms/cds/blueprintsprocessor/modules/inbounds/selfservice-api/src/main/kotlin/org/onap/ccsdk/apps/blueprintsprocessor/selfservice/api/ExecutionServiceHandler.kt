...
import org.onap.ccsdk.cds.blueprintsprocessor.services.workflow.BlueprintDGExecutionService
...
class ExecutionServiceHandler(private val bluePrintCoreConfiguration: BluePrintCoreConfiguration,
    ...
    fun response(executionServiceInput: ExecutionServiceInput, failure: Boolean = false): ExecutionServiceOutput {
        ...
        if (failure) {
            ...
            status.code = 500
            status.message = org.onap.ccsdk.cds.controllerblueprints.core.BluePrintConstants.STATUS_FAILURE
        } else {
            status.eventType = "EVENT-COMPONENT-PROCESSING"
            status.code = 200
            status.message = org.onap.ccsdk.cds.controllerblueprints.core.BluePrintConstants.STATUS_PROCESSING
        }
