...
import org.onap.ccsdk.cds.controllerblueprints.core.checkNotEmptyNThrow
    ...
    companion object {
        ...
        fun setResourceDataValue(resourceAssignment: ResourceAssignment, value: Any?) {
            ...
            try {
                if (checkNotEmpty(resourceProp.type)) {
                    ...
                    resourceAssignment.updatedBy = org.onap.ccsdk.cds.controllerblueprints.core.BluePrintConstants.USER_SYSTEM
                    resourceAssignment.status = org.onap.ccsdk.cds.controllerblueprints.core.BluePrintConstants.STATUS_SUCCESS
                }
            } catch (e: Exception) {
        ...
        fun setFailedResourceDataValue(resourceAssignment: ResourceAssignment, message: String?) {
            if (checkNotEmpty(resourceAssignment.name)) {
                resourceAssignment.updatedDate = Date()
                resourceAssignment.updatedBy = org.onap.ccsdk.cds.controllerblueprints.core.BluePrintConstants.USER_SYSTEM
                resourceAssignment.status = org.onap.ccsdk.cds.controllerblueprints.core.BluePrintConstants.STATUS_FAILURE
            ...
            try {
                ...
                assignments.forEach {
                    if (checkNotEmpty(it.name) && it.property != null) {
                        ...
                        } else if (org.onap.ccsdk.cds.controllerblueprints.core.BluePrintConstants.DATA_TYPE_STRING.equals(type, ignoreCase = true)) {
                            (root as ObjectNode).put(it.name, value as String)
                        } else if (org.onap.ccsdk.cds.controllerblueprints.core.BluePrintConstants.DATA_TYPE_BOOLEAN.equals(type, ignoreCase = true)) {
                            (root as ObjectNode).put(it.name, value as Boolean)
                        } else if (org.onap.ccsdk.cds.controllerblueprints.core.BluePrintConstants.DATA_TYPE_INTEGER.equals(type, ignoreCase = true)) {
                            (root as ObjectNode).put(it.name, value as Int)
                        } else if (org.onap.ccsdk.cds.controllerblueprints.core.BluePrintConstants.DATA_TYPE_FLOAT.equals(type, ignoreCase = true)) {
                            (root as ObjectNode).put(it.name, value as Float)
                        } else if (org.onap.ccsdk.cds.controllerblueprints.core.BluePrintConstants.DATA_TYPE_TIMESTAMP.equals(type, ignoreCase = true)) {
                            (root as ObjectNode).put(it.name, value as String)
                        } else {
