...
class RpcMessageUtilsTest {
    ...
    fun createHelloString() {
        ...
        var result = NetconfMessageUtils.createHelloString(capability).replace("[\n\r\t]".toRegex(), "")
        Assert.assertEquals(checkString, result)
    }
    ...
    fun validateChunkedFraming() {
        var reply = ("hello")
        var result = NetconfMessageUtils.validateChunkedFraming(reply)
    ...
    fun formatRPCRequest(){
        ...
        val capability = setOf<String>("hi", "hello","urn:ietf:params:netconf:base:1.1")

        var result = NetconfMessageUtils.formatRPCRequest(request,messageId,capability).replace("[\n\r\t]".toRegex(), "")
        Assert.assertEquals(checkString, result)
