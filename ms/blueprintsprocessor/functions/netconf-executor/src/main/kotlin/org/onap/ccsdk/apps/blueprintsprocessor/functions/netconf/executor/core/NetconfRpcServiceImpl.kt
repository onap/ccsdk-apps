...
class NetconfRpcServiceImpl(private var deviceInfo: DeviceInfo) : NetconfRpcService {
    ...
    override fun getConfig(filter: String, configTarget: NetconfDatastore): DeviceResponse {
    ...
    override fun deleteConfig(configTarget: NetconfDatastore): DeviceResponse {
        var output = DeviceResponse()
        val messageId = messageIdInteger.getAndIncrement().toString()
    ...
    override fun lock(configTarget: NetconfDatastore): DeviceResponse {
        var output = DeviceResponse()
        val messageId = messageIdInteger.getAndIncrement().toString()
    ...
    override fun unLock(configTarget: NetconfDatastore): DeviceResponse {
        var output = DeviceResponse()
        val messageId = messageIdInteger.getAndIncrement().toString()
    ...
    override fun editConfig(messageContent: String, configTarget: NetconfDatastore,
                            editDefaultOperation: ModifyAction): DeviceResponse {
        var response = DeviceResponse()
        val messageId = messageIdInteger.getAndIncrement().toString()
        log.info("$deviceInfo: editConfig: messageId($messageId)")
        try {
            val editMessage =
                NetconfMessageUtils.editConfig(messageId, configTarget, editDefaultOperation.action, messageContent)
    ...
    override fun validate(configTarget: NetconfDatastore): DeviceResponse {
        var output = DeviceResponse()
        val messageId = messageIdInteger.getAndIncrement().toString()
        try {
