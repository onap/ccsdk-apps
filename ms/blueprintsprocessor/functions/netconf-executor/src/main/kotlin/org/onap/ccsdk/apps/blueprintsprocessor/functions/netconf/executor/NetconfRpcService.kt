...
import org.onap.ccsdk.apps.blueprintsprocessor.functions.netconf.executor.data.NetconfExecutionRequest
...
class NetconfRpcService : NetconfRpcClientService {
    ...
    @Autowired
    fun NetconfRpcService(netconfDeviceInfo: DeviceInfo) {
        try {
            //Preconditions.checkNotNull(configResourceService, "missing configResourceService in netconf rpc client")
            //Preconditions.checkNotNull(configRestAdaptorService,
             //       "missing configRestAdaptorService in netconf rpc client")
            Preconditions.checkNotNull(netconfDeviceInfo, "missing netconfDeviceInfo in netconf rpc client")
            //Preconditions.checkNotNull(netconfExecutionRequest,
            //        "missing netconfExecutionRequest in netconf rpc client")

            this.deviceInfo = netconfDeviceInfo

           // connect(netconfDeviceInfo)
            //log.info("NetconfRpcClient initialised for netconf session {}", netconfSession.getDeviceInfo())
            ...
            throw NetconfException("Netconf Device Connection Failed,$netconfDeviceInfo",e)
        }
